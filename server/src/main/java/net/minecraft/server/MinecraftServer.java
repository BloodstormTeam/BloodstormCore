...
public abstract class MinecraftServer implements ICommandSender, Runnable, IPlayerUsage
{
    ...
    //public static ThermosConfig thermosConfig; //Crucible - Merge both configs inside one.
    ...
    public void tick()
    {
        ...
        CrucibleTimings.forgePreTick.startTiming();
        FMLCommonHandler.instance().onPreServerTick();
        CrucibleTimings.forgePreTick.stopTiming();
        ...
        this.theProfiler.endSection();
        callingForgeTick = true; // Cauldron start - handle loadOnProviderRequests during forge tick event
        CrucibleTimings.forgePostTick.startTiming();
        FMLCommonHandler.instance().onPostServerTick();
        CrucibleTimings.forgePostTick.stopTiming();
        callingForgeTick = false; // Cauldron end
    ...
    public void updateTimeLightAndEntities()
    {
        ...
        MinecraftTimings.bukkitSchedulerTimer.stopTiming(); // Crucible

        // Run tasks that are waiting on processing
        MinecraftTimings.processQueueTimer.startTiming(); //Crucible
        while (!processQueue.isEmpty())
        {
            processQueue.remove().run();
        }
        MinecraftTimings.processQueueTimer.stopTiming(); //Crucible

        MinecraftTimings.chunkIOTickTimer.startTiming(); //Crucible
        net.minecraftforge.common.chunkio.ChunkIOExecutor.tick();
        MinecraftTimings.chunkIOTickTimer.stopTiming(); //Crucible

        MinecraftTimings.timeUpdateTimer.startTiming(); //Crucible
        ...
        MinecraftTimings.timeUpdateTimer.stopTiming(); //Crucible
        ...
        for (int x = 0; x < ids.length; x++)
        {
            ...
            //{
                ...
                worldserver.timings.tracker1.startTiming(); // Spigot
                worldserver.getEntityTracker().updateTrackedEntities();
                worldserver.timings.tracker1.stopTiming(); // Spigot
                this.theProfiler.endSection();
                this.theProfiler.endSection();
            // } // CraftBukkit
        ...
        MinecraftTimings.connectionTimer.startTiming(); //Crucible
        this.func_147137_ag().networkTick();
        MinecraftTimings.connectionTimer.stopTiming(); //Crucible
        this.theProfiler.endStartSection("players");
        MinecraftTimings.playerListTimer.startTiming(); //Crucible
        this.serverConfigManager.sendPlayerInfoToAllPlayers();
        MinecraftTimings.playerListTimer.stopTiming(); //Crucible
        this.theProfiler.endStartSection("tickables");

        MinecraftTimings.tickablesTimer.startTiming(); //Crucible
        for (i = 0; i < this.tickables.size(); ++i)
        {
            ((IUpdatePlayerListBox)this.tickables.get(i)).update();
        }
        MinecraftTimings.tickablesTimer.stopTiming(); //Crucible
    ...
    {
        CrucibleServerMainHook.serverMain(p_main_0_);
